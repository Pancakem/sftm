...
import Path exposing (Path, PathNode(..))
...
init : Int -> Session -> Problem -> Model
init idx session problem =
    refocus [] problem (reallyInit idx session)


reallyInit : Int -> Session -> Model
reallyInit idx session =
    { problemIndex = idx
    , direction = History
    , historyIndex = 0
    , futureIndex = 0
...
view : Problem -> Model -> {title : String, content : Html Msg}
view problem model =
    let
        content = 
        ...
        f len idx1 idx2 step =
            li
                ...
                [ div [ class "history-gutter" ]
                    (if len < 0 then
                        [ text (String.fromInt (len + idx2)), span [ class "invisible" ] [ text "-" ] ]
                     else
                        [ text (String.fromInt (len - idx2)) ]
                    )
...
navBar model =
    div [ class "landscape-hide nav-bar", modelAriaHidden model ]
        ...
        , div [ class "top-title" ] [ text <| "Problem " ++ String.fromInt (model.problemIndex + 1) ]
    ...
    case msg of
        ...
        ResetPopUpResetMsg ->
            let
                ...
                newModel =
                    init model.problemIndex model.session newProblem
            in
