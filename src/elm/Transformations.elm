...
antecedents focus scratch context x =
    let
        f antecedents_ dct term result_ =
            case antecedents_ of
                [] ->
                    everyScratch
                        (\d -> focus /= substitute d (target x))
                        scratch
                        dct
                        result_

                ant :: ants ->
                    case unify term ant dct of
                        Nothing ->
                            result_

                        Just newDict ->
                            List.foldl (f ants newDict) result_ context
...
checkDictionary focus scratch targt dict =
    if dictionaryHasEmpty dict then
        case scratch of
            ...
            [ _ ] ->
                focus /= substitute (dictionaryAddScratch scratch dict) targt

            _ ->
                True
    else
        focus /= substitute dict targt
...
reallyAntecedents :
    ...
    -> Dictionary
    -> Dictionary
reallyAntecedents focus right scratch context antcedents dict =
    let
        f antecednts dct term result =
            case antecednts of
                [] ->
                    everyScratch2 (\d -> focus /= substitute d right) scratch dct result
    ...
    in
        List.foldr (f antcedents dict) dict context
